...
Vea la documentacion en ${prefix}/etc/ports/sources.conf mas abajo.
...
INSTALACION
	...
	Pasos para la Instalacion:
		...
		4. [opcional] editar ${prefix}/etc/ports/ports.conf
			El documento de configuracion ${prefix}/etc/ports/ports.conf define varias opciones de
		...
		5. [opcional] editar ${prefix}/etc/ports/sources.conf
			El documento ${prefix}/etc/ports/sources.conf muestra la locacion tanto de la copia local
...
escogido instalar DarwinPorts en algun otro lugar).
	...
	Si no esta seguro como y esta usando Panther (OS X 10.3), agrague la siguiente linea
a su documento de configuracion ~/.profile. Si el documento no existe, creelo. ( Esto tambien
se aplica si esta usando un shell bourne en Jaguar)

		export PATH=$PATH:/opt/local/bin

	Si esta usando Jaguar (OS X 10.2), agrague la siguiente linea a su documento de configuracion
~/.cshrc. (Esto tambien se aplica si esta usando tcsh en Panther)
...
comando "port" pero opera iterativamente en cada porte en dports/.
...
a un minimo.
	...
	Un Portfile consiste de sintaxis valida de TCL, evaluada por un interpretador de TCL inicializado
...
necesario.
	...
	Para comenzar, refierase a base/doc/exampleport y al manual portfile(7), o la Guia de DarwinPorts
localizada en http://darwinports.opendarwin.org/docs .
